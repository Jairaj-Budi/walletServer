...
export class WalletService implements OnApplicationShutdown, OnModuleDestroy {
  ...
  async createWallet(createWalletDto: CreateWalletDto): Promise<Wallet> {
    const wallet = await this.transactionManager.executeInTransaction(
      async (session) => {
        // Check if a wallet (user) with the same name already exists
        const existingWallet = await this.walletRepository.findByName(
        ...
        const newWallet = await this.walletRepository.createWithSession(
          walletData,
          session,
        );
  ...
  async loginUser(body) {
    const { name } = body;
    const getWalletData =
      await this.databaseTransactionService.findWalletDataByName(name);
    if (getWalletData?.length) {
      return getWalletData[0];
    } else {
      throw new InternalServerErrorException(
        'Wallet Data not found, please sign up',
      );
    }
  }
