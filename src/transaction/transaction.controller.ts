...
export class TransactionController {
  ...
  @ApiOperation({
    ...
    description:
      'Processes a transaction for a given wallet ID and updates the balance accordingly.',
  })
  ...
  async exportTransactions(
    ...
    try {
      const totalCount = await this.transactionService.getTransactionsCount(
        query.walletId,
      );
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename=transactions-${query.walletId}.csv`,
      );
      ...
      await this.transactionService.streamTransactionsToCSV(
        query.walletId,
        res,
      );
      ...
      if (!res.headersSent) {
        ...
        this.logger.error(
          `Export failed after streaming started: ${error.message}`,
          error.stack,
        );
        res.end();
      }
